...
//  Extension
//
//  Created by Eric Rado on 4/4/18.
...
class IntentHandler: INExtension, INRidesharingDomainHandling {
    ...
    func handle(intent: INRequestRideIntent, completion: @escaping (INRequestRideIntentResponse) -> Void) {
        ...
        // give it to the pickup and drop off location we already agreed with the user
        status.pickupLocation = intent.pickupLocation
        status.dropOffLocation = intent.dropOffLocation
        
        // mark it as confirmed - we will deliver the ride
        status.phase = INRidePhase.confirmed
        
        // say we will be there in `5minutes
        status.estimatedPickupDate = Date(timeIntervalSinceNow: 900)
        
        // create a new vehicle and configure it fully
        let vehicle = INRideVehicle()
        
        // workaround: load the car image into UIImage, convert that into PNG
        // data, then create an INImage out of that
        let image = UIImage(named: "car")!
        let data = UIImagePNGRepresentation(image)!
        vehicle.mapAnnotationImage = INImage(imageData: data)
        
        // set the vehicle's current location to wherever the user wants to go
        // this ought to at least place it a little way away for testing purposes
        vehicle.location = intent.dropOffLocation!.location
        
        // now that we have configured the vehicle fully, assign it
        // all at once to status.vehicle
    ...
    func resolvePickupLocation(for intent: INRequestRideIntent, with completion: @escaping (INPlacemarkResolutionResult) -> Void) {
        ...
        if let requestedLocation = intent.pickupLocation {
            // we have a valid pick up location - return sucess!
